...
{
	return decode((uint8_t *)outcome, str, strsize);
}

size_t base64_decode_overwrite(char *inout, size_t strsize)
{
	return decode((uint8_t *)inout, inout, strsize);
}
...
size_t base64_encode(char *outcome, const void *data, size_t datasize)
{
	...
	for (size_t i = 0; i < datasize; i += 3) {
		...
		outcome[outdex++] = to_base64(word, 3);
		outcome[outdex++] = to_base64(word, 2);
		outcome[outdex++] = c1? to_base64(word, 1) : '=';
		outcome[outdex++] = c2? to_base64(word, 0) : '=';
	}
